...
public class ProductSearchService {
        ...
        public ProductSearchResponse getProducts(
                ...
                try {
                        ...
                        List<co.elastic.clients.elasticsearch._types.SortOptions> sortOptions = parseSortFromPageable(
                                        pageable);
                        ...
                        PagedResponse<ProductDTO> pagedProducts = PagedResponse.<ProductDTO>builder()
                                        ...
                                        .last((long) (pageable.getPageNumber() + 1)
                                                        * pageable.getPageSize() >= totalHits)
                                        .hasNext((long) (pageable.getPageNumber() + 1)
                                                        * pageable.getPageSize() < totalHits)
        ...
        private ProductFacetsDTO extractFacets(
                ...
                if (response.aggregations().containsKey("leaf_categories")) {
                        facets.setCategories(
                                        response.aggregations().get("leaf_categories").sterms().buckets().array()
                                                        .stream()
                                                        .map(bucket -> {
                        ...
                        images = doc.getVariants().stream()
                                        ...
                                        .map(img -> ProductImageDTO.builder()
                                                        ...
                                                        .imageRole(img.getImageRole() != null ? 
                                                                com.nala.armoire.model.entity.ImageRole.valueOf(img.getImageRole()) : null)
                                                        .build())
                                        .collect(Collectors.toList());
                }

                // Fallback: If variants don't have images, check product-level (backward
                // compatibility)
